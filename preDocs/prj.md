...
| 指标类别 | 具体指标 | 实测值 |
|---|---|---|
| **Kernel 级** | R-SWA Attention kernel 耗时（W=128, batch=1） | 见 `bench_cuda_decode` 的 `fwd` 分量 |
| | MoE INT4 GEMM 有效 TFLOPS（batch=8） | 未测（当前 Engine 为单请求） |
| | CUDA Graph launch 开销 | 13 层 kernel 序列：plain 6.8 → graph 5.4 ms/token |
...
| **吞吐** | batch=8 输出 token 吞吐 | INT4 269 tok/s / BF16 272 tok/s（prompt64，ragged prefill + bf16 TC GEMM） |
| | batch=16 输出 token 吞吐 | INT4 374 tok/s / BF16 417 tok/s（同上；见 `BENCHMARKS.md` §2.7） |
